...
impl BitVector<u64> {
    ...
    fn from_bytes(bytes: Vec<u8>, width: u64, signed : bool) -> BitVector<u64> {
        let mut ans: BitVector<u64> = BitVector { width: width, bits: Vec::new() };
        ans.reserve(width);
        for i in 0 .. width {
    ...
    fn add(left : BitVector<u64>, right : BitVector<u64>) -> BitVector<u64> {
        ...
        let ans : Vec<u8> = num_bigint::BigInt::from_signed_bytes_le(left_bytes.as_ref()).add(
            num_bigint::BigInt::from_signed_bytes_le(right_bytes.as_ref())
        ).to_signed_bytes_le();
